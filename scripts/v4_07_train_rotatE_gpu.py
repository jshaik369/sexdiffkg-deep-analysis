#!/usr/bin/env python3
"""
RotatE v4.1 on GPU — 200 epochs, single train() call.
Use AFTER DistMult v4.1 finishes to reclaim GPU.
"""
import json
import time
import torch
...
print("=" * 60)
print("RotatE v4.1 Training on GPU (200 epochs, single call)")
print("=" * 60)
...
print(f"\nTraining 200 epochs (single call, no block splitting)...")
t0 = time.time()
...
print(f"\n{'=' * 60}")
print(f"RotatE v4.1 GPU Results")
print(f"{'=' * 60}")
print(f"MRR:      {mrr:.5f}")
...
print(f"Train:    {train_time/60:.1f} min ({train_time/3600:.1f} h)")

metrics = {
    "model": "RotatE_v4.1_GPU",
    "kg_version": "v4.1",
    ...
    "lr": 0.00005,
    "neg_samples": 256,
    "loss": "NSSALoss(margin=9.0, adv_temp=1.0)",
    "mrr": mrr,
    "hits_at_1": h1,
    "hits_at_10": h10,
    "amr": amr,
...
print(f"\nMetrics saved to {OUT}/rotatE_v41_gpu_metrics.json")
